...
Versión limpia: v1 · 2026-06-12 · Fuentes verificadas

**Módulo 4 — El sistema** · **Semana 7 de 8**

**Tiempo estimado real del alumno:** ~5 h
...
La Semana 8 cierra la prueba final — por eso esta semana empiezas a leer el caso.
...
Un SDR que opera un **sistema** piensa distinto. Diseña una arquitectura donde el volumen es función de la configuración, no del esfuerzo diario. Define sus ICPs, sus secuencias, sus cadencias, sus canales. Después alimenta el sistema y vigila las métricas. El trabajo cotidiano es **mantener el sistema corriendo y ajustar cuando hace falta**, no "decidir cada prospecto".
...
Una secuencia es **un conjunto ordenado de toques multicanal con reglas de ramificación según la respuesta del prospecto**.
...
**Regla 3 — Acorde a la velocidad del prospecto.** Un C-level demora más en responder que un mando medio. Un sector regulado (servicios financieros) se mueve más lento que una startup. La cadencia se ajusta al tempo del prospecto, no al tuyo.
...
- **Acercamiento:** vía referidores dentro de la empresa. Buscas champions potenciales (un mando medio del área, uno o dos niveles debajo del decisor) y les pides orientación, **no les vendes**.
...
Puedes escribir el mejor correo en frío del mundo. Si no llega a la bandeja de entrada del prospecto, nada de lo anterior sirve. La **entregabilidad** (*deliverability*) es que tus correos efectivamente aterricen en la bandeja de entrada y no en spam ni en promociones.
...
**2. Calentamiento.** Un dominio nuevo **no puede mandar envíos masivos desde el primer día**. Necesita calentarse: empezar con poco volumen, interactuar con casillas que responden, e ir subiendo de a poco. Si te saltas el calentamiento, el dominio se quema enseguida y deja de servir.
...
**4. Autenticación técnica.** SPF, DKIM y DMARC son registros que le dicen a los proveedores "este dominio es legítimo". Sin ellos, los correos caen a spam casi automáticamente. Los configura el equipo técnico, pero tú entiendes que existen y para qué sirven.
...
- **Concepto:** marcador (dialer) para llamadas con registro en el CRM. **Herramientas:** Aircall, Dialpad, JustCall, CloudTalk.
...
- **Concepto:** monitoreo de entregabilidad y reputación. **Herramientas:** Postmaster Tools, MXToolbox, GlockApps.
...
- El **proceso outbound de punta a punta** (desde el ICP hasta el handoff).
...
## Semana 7 — Micro-pruebas

### Micro-pruebas de la Semana 7 (La maquinaria del outbound)

**Tiempo del alumno:** ~20 min (4 preguntas + explicación inmediata después de cada una). **Corrección automática.**

────────────────────────────────────────

#### Reglas

- 1 solo intento, sin recuperación.
- Tiempo acotado por pregunta.
- No se puede volver atrás.
- El orden de las preguntas y de las opciones cambia para cada persona.
- Después de cada respuesta ves de inmediato la correcta y por qué.

**Umbral:** menos de 65% acumulado en las micro-pruebas del módulo levanta una bandera roja.

────────────────────────────────────────

#### Pregunta 1 — Diagnóstico: qué variante de acceso al decisor aplica (120 seg)

##### Contexto

Estás prospectando a **Consultora Andina**, una empresa peruana de consultoría de transformación digital, 250 empleados. Tu ICP es claro: el decisor es la VP de Nuevos Negocios.

Revisas tu base y LinkedIn y confirmas:

- La VP de Nuevos Negocios es Patricia Mendoza. Su correo corporativo **no aparece** en tu base ni en la web de la empresa. Su teléfono tampoco.
- Encuentras **4 coordinadores del área comercial** (con correo verificado) y **2 gerentes de cuenta** (uno con correo y teléfono).
- Consultora Andina nunca fue contactada antes.

##### Pregunta

¿Qué variante de acceso al decisor corresponde a esta situación?

a) Variante 1 — multicanal proactivo directo a Patricia (correo + llamada).
b) Variante 2 — correo paciente a Patricia con cadencia espaciada.
c) Variante 3 — vía referidores dentro de la empresa, buscando champions en los mandos medios. **(correcta)**
d) Variante 5 — recontacto con ángulo nuevo después de una ventana de enfriamiento.

##### Respuesta correcta: c

**Por qué c es correcta:** el criterio de diagnóstico (Bloque 6) es explícito: si **no tienes contacto verificado del decisor** y la empresa nunca fue contactada, la variante es la 3 — acceso vía referidores dentro de la empresa. Los 4 coordinadores y 2 gerentes son candidatos a champions: les pides orientación ("¿quién vería este tema en su organización?"), **no les vendes**. Es la variante con la que más tiempo pasa un SDR típico y la que más criterio exige, porque el objetivo del mensaje es distinto (conseguir que te ruteen, no agendar una reunión).

**Por qué los distractores son incorrectos:**

- **a (variante 1):** requiere **correo y teléfono verificados del decisor**. Patricia no tiene ninguno de los dos. Mandarle correo a una dirección inventada es baja probabilidad y muchos rebotes = quema la reputación del dominio.
- **b (variante 2):** requiere al menos **correo verificado del decisor**. No lo tienes. Esta variante aplica cuando hay correo del decisor pero no teléfono — no es este caso.
- **d (variante 5):** aplica solo cuando **ya contactaste antes sin éxito**. La empresa nunca fue contactada. No hay ventana de enfriamiento que respetar.

**La regla detrás de la pregunta:** el error clásico del novato es asumir que tiene que "llegarle al decisor sí o sí desde el primer mensaje". La variante 3 acepta que el camino es indirecto y no lo ve como un premio de consuelo — en el outbound B2B real, es el camino más frecuente.

────────────────────────────────────────

#### Pregunta 2 — Selección múltiple: identifica los errores de una secuencia propuesta (120 seg)

##### Contexto

Un SDR te pasa esta secuencia para revisar. Está diseñada para un decisor con correo y teléfono verificados (variante 1):

| Paso | Día | Canal | Acción |
| --- | --- | --- | --- |
| 1 | 0 | WhatsApp | Mensaje inicial presentándose |
| 2 | 0 | Correo | Primer correo con gancho |
| 3 | 1 | Teléfono | Llamada en frío |
| 4 | 1 | LinkedIn | Mensaje contado como toque de la cadencia |
| 5 | 2 | Correo | Seguimiento |
| 6 | 3 | Teléfono | Segunda llamada |
| 7 | 4 | WhatsApp | Empujón |
| 8 | 5 | Correo | Último correo |

##### Pregunta

Marca **todos los errores estructurales** que tiene esta secuencia:

- [ ] 1. Arranca con WhatsApp en frío como primer toque. **(correcta — WhatsApp no es canal de entrada)**
- [ ] 2. Usa el correo como segundo canal en vez de primero.
- [ ] 3. 8 toques comprimidos en 5 días = saturación. **(correcta — cadencia demasiado pegada)**
- [ ] 4. Cuenta LinkedIn como toque de la cadencia en vez de dejarlo en paralelo (investigación + micro-contacto). **(correcta — contradice la lógica del orden)**
- [ ] 5. La llamada del día 1 aparece antes del seguimiento por correo — falta acumulación de contexto escrito. **(correcta — rompe la lógica del orden)**
- [ ] 6. Usa el teléfono dos veces en la misma secuencia.
- [ ] 7. No declara ninguna regla de ramificación según la respuesta del prospecto. **(correcta — toda secuencia necesita la regla "qué pasa si responde")**

##### Respuestas correctas: 1, 3, 4, 5, 7

**Por qué 1 es error (Bloque 5):** WhatsApp es **empujón tras un contacto previo**, no entrada. En frío, el prospecto lo lee como invasión de su espacio personal — riesgo alto de bloqueo. Y una vez que te bloquea, no accedes nunca más.

**Por qué 3 es error (Bloque 4):** la cadencia sana son pocos toques bien espaciados, dentro de una ventana de un par de semanas. Meter 8 toques en 5 días rompe la regla "ni tan pegado que incomode" por mucho. El prospecto se siente perseguido; lo más probable es que te reporte como spam o te bloquee.

**Por qué 4 es error (Bloque 5):** el Bloque 5 es explícito — LinkedIn corre en paralelo, **sin contar como toque de la cadencia**. Numerarlo como toque de la secuencia lo trata como un canal de cadencia más, justo lo que la lección dice que no hagas. Conectar e investigar por LinkedIn está bien; meterlo como paso numerado de la cadencia, no.

**Por qué 5 es error (Bloque 5):** la lógica del orden es correo → llamada. El correo primero **introduce contexto sin imponer presencia** y establece el nombre del remitente. Cuando entra la llamada, el prospecto ya vio el nombre — deja de ser una interrupción al azar. Llamar el día 1, con un único correo enviado horas antes, no deja que el contexto aterrice.

**Por qué 7 es error (Bloque 4):** una secuencia sin reglas de ramificación no es secuencia — es una lista. Toda secuencia real declara: si el prospecto responde, la secuencia automática se pausa y el SDR toma el control manual. Sin esa regla, el prospecto puede responder en el toque 3 y seguir recibiendo toques automáticos hasta el 8 — la peor experiencia posible.

**Por qué los otros NO son errores estructurales:**

- **2 (WhatsApp antes que el correo):** el error real es WhatsApp como entrada (ítem 1), no "el correo como segundo canal". Este ítem confunde la causa.
- **6 (teléfono dos veces):** es correcto usar el teléfono dos veces — el Bloque 5 justamente propone una llamada en el toque 3 y una última llamada al final. El error no es repetir el canal, sino la cadencia comprimida.

────────────────────────────────────────

#### Pregunta 3 — Pareo: concepto vs herramienta (90 seg)

##### Contexto

El SDR tiene que entender que las herramientas instrumentan un concepto más fundamental. Si cambia de empresa, las herramientas cambian pero el concepto no.

##### Pregunta

Empareja cada concepto con el grupo de herramientas que lo instrumentan:

| # | Concepto |
| --- | --- |
| 1 | Base de datos de prospectos con filtros de ICP y señales |
| 2 | Motor de secuencias multicanal con reglas de ramificación |
| 3 | Monitoreo de entregabilidad y reputación de dominio |
| 4 | Calentamiento automatizado de dominios nuevos |

| Letra | Grupo de herramientas |
| --- | --- |
| A | MailReach, Instantly Warmup, Smartlead Warmup |
| B | Apollo, ZoomInfo, Crunchbase, Lusha |
| C | Google Postmaster Tools, MXToolbox, GlockApps |
| D | Reply.io, Outreach, Salesloft, Instantly, Smartlead |

##### Respuestas correctas

- **1 → B** (Apollo, ZoomInfo, Crunchbase y Lusha son bases de datos de prospectos con filtros de ICP).
- **2 → D** (Reply.io, Outreach, Salesloft, Instantly y Smartlead son motores de secuencias multicanal).
- **3 → C** (Google Postmaster, MXToolbox y GlockApps monitorean reputación y entregabilidad).
- **4 → A** (MailReach, Instantly Warmup y Smartlead Warmup automatizan el calentamiento de dominios).

**La regla de carrera (Bloque 8):** cuando entres a un trabajo nuevo con otras herramientas (por ejemplo, ZoomInfo + Outreach en vez de Apollo + Reply.io), los conceptos son idénticos. Transfieres el conocimiento en una semana, aprendiendo nada más la interfaz nueva. Por eso esta pregunta evalúa el mapa conceptual, no los nombres.

────────────────────────────────────────

#### Pregunta 4 — Caída de la tasa de respuesta: diagnóstico por descarte (120 seg)

##### Contexto

Eres SDR. Tu tasa de respuesta (reply rate) venía estable durante semanas. En las últimas **dos semanas**, sin cambios en el mensaje, sin cambios en el ICP y sin cambios en la cadencia, cayó en seco.

Tu líder de equipo te pide una hipótesis y un checklist de verificación.

##### Pregunta

¿Cuál es la hipótesis **más probable** y cuál debería ser el **primer paso**?

a) El mensaje se saturó en el mercado — hipótesis: fatiga de mensaje; primer paso: rediseñar el asunto y probar variantes (A/B test).
b) El ICP está mal calibrado — hipótesis: los prospectos nuevos no encajan; primer paso: revisar los filtros y ajustar industria/cargo.
c) Problema de entregabilidad — hipótesis: uno o más dominios se quemaron o la reputación cayó; primer paso: revisar la salud de envío de los dominios (rebotes, quejas de spam, reputación) antes de tocar el mensaje. **(correcta)**
d) Problema de volumen — hipótesis: los prospectos actuales ya recibieron secuencias previas; primer paso: pedir una lista nueva al área de investigación.

##### Respuesta correcta: c

**Por qué c es correcta:** la pregunta te da tres variables fijas (mensaje, ICP, cadencia) y una caída brusca. Cuando nada cambió en el mensaje ni en el receptor, pero la respuesta se desploma, la explicación más probable es que **los mensajes no están llegando a la bandeja de entrada**: caen a spam, a promociones, o directamente rebotan. El primer paso (Bloque 7) es confirmar que el problema es del canal y no del mensaje, revisando la salud de envío de los dominios. Lo demás —cambiar el copy, ajustar el ICP— viene solo después de descartar la entregabilidad.

**Por qué los distractores son trampas típicas:**

- **a (fatiga de mensaje):** es lo que hace el novato cuando ve caer la respuesta. Pero el problema dice explícitamente que el mensaje **no cambió**. Y si la causa fuera la fatiga, la caída sería gradual, no en seco en dos semanas. Rediseñar el mensaje antes de revisar la entregabilidad es probar soluciones sobre un canal roto — no sirve.
- **b (ICP mal calibrado):** se descarta por la misma lógica — el ICP no cambió. Un ICP que venía funcionando no se vuelve de golpe mal encaje.
- **d (lista contaminada):** puede ser un factor menor, pero no explica una caída tan brusca. Y el error del novato es proponer una solución ("lista nueva") antes del diagnóstico. El Bloque 7 es explícito: **diagnóstico antes que solución**.

**La lección del caso:** el novato, ante una caída de métricas, tiende a "probar cosas" en las variables que sí controla (mensaje, asuntos). Pero si el problema está en el canal (la entregabilidad), ninguna prueba en el mensaje va a mover la aguja. **Hasta que no se resuelva el canal, lo demás no importa.** Este es el mindset que se profundiza en la Semana 8: diagnóstico por descarte antes que acción por intuición.

────────────────────────────────────────

#### Fin de las micro-pruebas de la Semana 7

Con esto llegas al borde de la Semana 8. Lo que falta:

- **Resto de la Semana 7:** seguir con la lectura del brief del caso de la prueba final y su primer análisis. El caso es largo — esta semana te da espacio para leerlo sin apuro antes del cierre.
- **Semana 8:** proceso de punta a punta, métricas, diagnóstico por descarte y mindset de mejora continua, más la prueba del módulo, la prueba final del curso y la sesión en vivo de cierre.
