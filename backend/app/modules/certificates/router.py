...
def issue(
    ...
    """Admin fuerza la emisión (útil para reemisiones o piloto)."""
    cert = services.issue_if_eligible(db, user_id=user_id, cohort_id=cohort_id)
    if cert is None:
        log.warning(
    ...
    log.info(
        "certificate.issued_by_admin",
        extra={"certificate_id": cert.id, "user_id": user_id, "cohort_id": cohort_id},
    )
    return cert
