"""Seed: Prueba del Módulo 2 — Capa 2.
...
Asume que el Módulo 2 ya existe en DB (creado por seed_w3.py). Idempotente.

Uso:
    cd backend && .venv/bin/python -m app.scripts.seed_capa2_m2
"""
...
configure_logging()
log = get_logger("seed_capa2_m2")

MODULE_SLUG = "m2-el-otro-lado"
ASSESSMENT_TITLE = "Prueba del Módulo 2 (Capa 2)"
...
def run() -> None:
    ...
    try:
        ...
        if module is None:
            raise RuntimeError(
                f"Module '{MODULE_SLUG}' no existe. Corré seed_w3 antes."
            )
