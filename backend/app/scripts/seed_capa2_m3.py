"""Seed: Prueba del Módulo 3 — Capa 2.
...
Asume que el Módulo 3 ya existe en DB (creado por seed_w5.py). Idempotente.

Uso:
    cd backend && .venv/bin/python -m app.scripts.seed_capa2_m3
"""
...
configure_logging()
log = get_logger("seed_capa2_m3")

MODULE_SLUG = "m3-la-conexion"
ASSESSMENT_TITLE = "Prueba del Módulo 3 (Capa 2)"
...
def run() -> None:
    ...
    try:
        ...
        if module is None:
            raise RuntimeError(
                f"Module '{MODULE_SLUG}' no existe. Corré seed_w5 antes."
            )
