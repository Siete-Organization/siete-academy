"""Tests para Capa 2 Módulo 4 — contenido + seed + auto_grade integration."""
import pytest
...
from app.scripts.capa2_m4_questions import (
    MCQ,
    PASSING_SCORE,
    VIDEO_CASES,
    VIDEO_RUBRIC,
...
from app.scripts.seed_capa2_m4 import _upsert_capa2, MODULE_SLUG


@pytest.fixture
def module_m4(db) -> Module:
...
class TestContent:
    ...
    def test_ids_use_m4_prefix(self):
        for q in MCQ:
            assert q["id"].startswith("M4."), q["id"]
    ...
    def test_sections_are_sem7_or_sem8(self):
        for q in MCQ:
            assert q.get("section") in ("sem7", "sem8"), q["id"]
...
class TestSeed:
    def test_upsert_creates_capa2_assessment(self, db, module_m4):
        a = _upsert_capa2(db, module_m4)
        assert a.type == "capa_2"
        assert a.lesson_id is None
        assert a.module_id == module_m4.id
        assert a.passing_score == PASSING_SCORE
        assert len(a.config["questions"]) == 16
        assert len(a.config["video_cases"]) == 4

    def test_upsert_is_idempotent(self, db, module_m4):
        a1 = _upsert_capa2(db, module_m4)
        a2 = _upsert_capa2(db, module_m4)
        assert a1.id == a2.id


class TestAutoGradeIntegration:
    def test_auto_grade_all_correct(self, db, module_m4):
        a = _upsert_capa2(db, module_m4)
